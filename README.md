...
* オペレーティングシステム:
    ...
    * Amazon Linux 2
* Python バージョン:
    * Python 3.8 : v1.6.9 以降を新規インストールした場合
    * Python 3.6 : v1.6.8 以前からアップデートした場合
* ネットワークインタフェース: 1つ以上

※ CentOS 8 系は EOL に伴いサポート外となりました（インストールに失敗します）。CentOS Stream 8 または RHEL 互換 OS のご利用をご検討ください。
...
* CentOS/RHEL 7.6 でインストールに失敗する場合、Python 3.8 または 3.6 を事前にインストールしておき、install.sh のオプションに `--skip-python3-install` を指定して下さい。
...
### 4.3. 冗長構成での注意点
- v.1.6.6 以降で冗長構成時のアップデート手順が変更され、install.sh の後の pcs cluster start が不要になりました。詳しくは管理ガイドの「1.9.3.アップデート」をご参照ください。
